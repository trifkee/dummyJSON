...
function AllPosts() {
    ...
    const ifFetchPosts = () => {
        ...
        return allPosts?.data?.posts.map((post:SinglePost) => <Post key={post.id} props={post} />)
        // return allPosts?.data.posts.map((post:SinglePost) => <Post key={post.id} props={post})
    }
    ...
    return (
        ...
        <section className='posts-section' style={{display:'flex', marginBottom:'2rem'}}>
                <div className="users-list">
                    {allUsers?.data?.users.map((user:User) => <Link className='user-list' to={`/profile/${user.id}`} key={user.id}><img src={user.image} alt={user.username}/> <p>{user.username}</p></Link>)}
                </div>
